...
base = alt.Chart(bal).encode(
    x=alt.X("الشهر:N", title=None, axis=alt.Axis(labelColor="#1A2433", labelAngle=-40)),
    y=alt.Y("الرصيد:Q", title="ريال", axis=alt.Axis(labelColor="#1A2433", titleColor="#6C6E79", gridColor="rgba(0,0,0,.10)")))
area = base.mark_area(line={"color": "#2E86C1", "strokeWidth": 3}, color=alt.Gradient(gradient="linear",
        stops=[alt.GradientStop(color="rgba(46,134,193,0)", offset=0), alt.GradientStop(color="rgba(46,134,193,.45)", offset=1)],
        x1=1, x2=1, y1=1, y2=0))
points = base.mark_circle(size=55, color="#2E86C1")
zero = alt.Chart(pd.DataFrame({"y": [0]})).mark_rule(color="#C0392B", strokeDash=[4, 3]).encode(y="y:Q")
chart = (area + points + zero).properties(height=200, background="transparent").configure_view(strokeWidth=0)
